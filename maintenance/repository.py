...
#         """Check if a bill exists for a specific flat, month and year."""
#         conn = get_db_connection()
#         cur = conn.cursor(cursor_factory=RealDictCursor)
#         try:
#             cur.execute("""
...
#         """Mark a specific bill as paid manually (cash collection)."""
#         conn = get_db_connection()
#         cur = conn.cursor()
#         try:
#             cur.execute("""
...
class MaintenanceRepository:
    ...
    # ── NEW: Required by collect_maintenance route ──────────────────────────
    ...
    def get_by_flat_id_month_year(flat_id, month, year):
        ...
        cur = conn.cursor(cursor_factory=RealDictCursor)
        try:
            cur.execute("SELECT * FROM maintenance WHERE flat_id = %s AND month = %s AND year = %s",
                        (flat_id, month, int(year)))
            return cur.fetchone()
        finally:
            cur.close(); conn.close()
    ...
    def mark_as_paid_manually(flat_id, month, year):
        ...
        cur = conn.cursor()
        try:
            cur.execute("""UPDATE maintenance SET status='paid', paid_date=CURRENT_DATE,
                        payment_method='Cash' WHERE flat_id=%s AND month=%s AND year=%s""",
                        (flat_id, month, int(year)))
            conn.commit()
            return True
        finally:
